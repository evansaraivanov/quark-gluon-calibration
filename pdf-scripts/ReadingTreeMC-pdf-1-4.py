...
finput = TFile.Open("dijet_sherpa_bdt.root")
			...
			if abs(i.j1_eta) > abs(i.j2_eta):
				...
				eta2 = "Central"
                        ...
                        for j in range(1,4):
                            ...
                            FillHisto(str(pTbin1)+"_LeadingJet_"+eta1+"_"+label1+str(j), JetList[0], weight_all)
                            FillHisto(str(pTbin2)+"_SubJet_"+eta2+"_"+label2+str(j), JetList[1], weight_all)


#foutput = TFile("dijet-data-py.root","recreate")
foutput = TFile("dijet-sherpa-py-1-4.root","recreate")
