...
bins = [0.0,0.5,1.0,2.1]  #this bin range is for only dijet event
...
for i in t1:
	if i.pass_HLT_j400 == 1 :#and i.j1_is_truth_jet and i.j2_is_truth_jet:
		if i.j1_pT > 500 and i.j1_pT < 600 and abs(i.j1_eta) < 2.1 and abs(i.j2_eta) < 2.1 and i.j1_pT/i.j2_pT < 1.5:
...
foutput = TFile("dijet-sherpa-eta-500-600-pt.root","recreate")
